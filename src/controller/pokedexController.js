...
const createPokemon = async (req, res) => {
  try {
    ...
    if (!coachId) {
      return res.status(400).json({message: 'É obrifatório o id do treinador'})
    }
    
    const findCoach = await CoachModel.findById(coachId)

    if (!findCoach) {
      return res.status(404).json({message: 'O treinador não foi encontrado'})
    }
...
// Verifica se o coachID rcebido existe
// verifica se o dado recebido é valido
...
const deletePokemonById = async(req, res) => {
  try {
    const findPokemon = await PokedexModel.findById(req.params.id)
    
    if (findPokemon == null) {
      return res.status(404).json({message: `O pokemon com o id ${req.params.id} não foi encontrado`})
    }

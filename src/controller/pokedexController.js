...
const createPokemon = async (req, res) => {
    try {
        ...
        if (!findCoach) {
            return res.status(404).json({ message: ' Treinador não foi encontrato' })
        }

// new pokedexModel -> a gente gera um novo MODELO de um pokemon com base na Schema
        const newPokemon = new PokedexModel({
            coach: coachId,
            name, type, abilities, description
        })

// db.pokedexes.insertOne(newPokemon)
...
//2. verificar se coachId recebido existe

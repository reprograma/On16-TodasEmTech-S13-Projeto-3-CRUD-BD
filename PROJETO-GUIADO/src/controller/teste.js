...
const deletePokemonById = async (req, res) => {
  ...
  const deletedPokemon = await PokedexModel.findByIdAndDelete(id)
  const message = `Pokemon with ID ${id} has been successfully deleted!`

  res.status(200).json({message, deletedPokemon})
} catch (error) {
  console.error(error)
  res.status(500).json({message: error.message})
}}
